...
__ALIGN_BEGIN static uint8_t HID_EXK_ReportDesc[HID_EXK_REPORT_DESC_SIZE] __ALIGN_END =
{
  ...
  0x09, 0x02,               // Usage (Mouse)
  0xA1, 0x01,               // Collection (Application)
  0x85, REPORT_ID_MOUSE,    // ★ enum 상수 사용 (컴파일 시 0x02로 변환됨)
  0x09, 0x01,               //   Usage (Pointer)
  0xA1, 0x00,               //   Collection (Physical)
  0x05, 0x09,               //     Usage Page (Button)
  0x19, 0x01,               //     Usage Minimum (0x01)
...
static uint32_t rate_sof_cnt = 0; 
...
bool usbHidSendMouseReport(uint8_t buttons, int8_t x, int8_t y, int8_t wheel)
{
  ...
  buffer[0] = REPORT_ID_MOUSE; // 에넘에 의해 자동으로 2가 대입됩니다.
  buffer[1] = buttons;         
  buffer[2] = (uint8_t)x;      
  buffer[3] = (uint8_t)y;      
  ...
  if (args->argc >= 1 && args->isStr(0, "rate") == true)
  {
    ...
    {      
      if (millis()-pre_time >= 1000)
      {
        ...
        cliPrintf("hid rate %d Hz, sof %d Hz\n", 
          data_in_rate,
          rate_sof_cnt
          ); 
