...
bool swtimerInit(void)
{
  ...
  {
    swtimer_tbl[i].Timer_En   = OFF_SWTIMER;
...
void swtimerISR(void)
{
  ...
  {
    if ( swtimer_tbl[i].Timer_En == ON_SWTIMER)                         // 타이머가 활성화 됬니?
    {
      ...
      if (swtimer_tbl[i].Timer_Ctn == 0)                        // 타이머 오버플로어
      {
        ...
        {
          swtimer_tbl[i].Timer_En = OFF_SWTIMER;                        // 타이머 OFF 한다.
        }
...
void swtimerStart(swtimer_handle_t TmrNum)
{
  ...
  swtimer_tbl[TmrNum].Timer_En  = ON_SWTIMER;
}
...
void swtimerStop (swtimer_handle_t TmrNum)
{
  ...
  swtimer_tbl[TmrNum].Timer_En = OFF_SWTIMER;
}
